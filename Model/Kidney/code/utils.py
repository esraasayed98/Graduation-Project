...
def rnaseqdb_df_Kidney():

    data = pd.read_csv('../data/KIRC_mRNA_core.csv',index_col=[0])

    expr_data = data.values
    All_gene_symbols =(data.columns.values).tolist()

    selected_gene = []

    with open('..//data/kidney Passway.txt') as f:
        for line in f:
            selected_gene.append(line.strip())

       
    all_genes=[]
    for gene in (All_gene_symbols):
        gene_val=(((gene.split('_'))[-1]).split('|'))[0]
    ...
    for i in selected_gene:
            try:
                gene_idxs.append(all_genes.index(i))
                symb.append(i)
            except ValueError:
                pass
